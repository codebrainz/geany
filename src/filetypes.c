...
static inline void ft_init(filetype_id ft_id, int lang, const char *name,
	...
	GeanyFiletypeGroupID group_id)
{
	...
	if (mime_type != NULL)
		ft->mime_type = g_strdup(mime_type);
	ft->group = group_id;
}
...
static void init_builtin_filetypes(void)
{
	 ...
	 * --------------------------------------------------------------------------------------------------------------------------
	 *       [0]        [1]  [2]                 [3]                        [4]          [5]                           [6]      */
	FT_INIT( NONE,       -1, "None",             _("None"),                 NONE,        NULL,                         NONE     );
	...
	FT_INIT( CAML,       -1, "CAML",             "(O)Caml",                 SOURCE_FILE, "text/x-ocaml",               COMPILED );
	...
	FT_INIT( XML,        -1, "XML",              NULL,                      DOCUMENT,    "application/xml",            MARKUP   );
	...
	FT_INIT( LISP,       -1, "Lisp",             NULL,                      SOURCE_FILE, NULL,                         SCRIPT   );
	FT_INIT( ERLANG,     -1, "Erlang",           NULL,                      SOURCE_FILE, "text/x-erlang",              COMPILED );
	FT_INIT( CONF,       10, "Conf",             _("Config"),               FILE,        NULL,                         MISC     );
	FT_INIT( PO,         -1, "Po",               _("Gettext translation"),  FILE,        "text/x-gettext-translation", MISC     );
	...
	FT_INIT( YAML,       -1, "YAML",             NULL,                      FILE,        "application/x-yaml",         MISC     );
	FT_INIT( CMAKE,      -1, "CMake",            NULL,                      SOURCE_FILE, "text/x-cmake",               SCRIPT   );
	FT_INIT( NSIS,       35, "NSIS",             NULL,                      SOURCE_FILE, NULL,                         SCRIPT   );
	FT_INIT( ADA,        -1, "Ada",              NULL,                      SOURCE_FILE, "text/x-adasrc",              COMPILED );
	FT_INIT( FORTH,      -1, "Forth",            NULL,                      SOURCE_FILE, NULL,                         SCRIPT   );
	FT_INIT( ASCIIDOC,   43, "Asciidoc",         NULL,                      SOURCE_FILE, NULL,                         MARKUP   );
	FT_INIT( ABAQUS,     44, "Abaqus",           NULL,                      SOURCE_FILE, NULL,                         SCRIPT   );
	FT_INIT( BATCH,      -1, "Batch",            NULL,                      SCRIPT,      NULL,                         SCRIPT   );
	FT_INIT( POWERSHELL, -1, "PowerShell",       NULL,                      SOURCE_FILE, NULL,                         SCRIPT   );
	...
	foreach_strv(name, names)
	{
		...
		if (ft)
		{
				...
				(group_id == GEANY_FILETYPE_GROUP_COMPILED || group_id == GEANY_FILETYPE_GROUP_SCRIPT))
			{
				...
				filetype_make_title(ft->name, TITLE_SOURCE_FILE);
			}
		}
		else
