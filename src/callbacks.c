...
{
	UTILS_REPLACE_PTR(search_data.text, g_strdup(text));
	UTILS_REPLACE_PTR(search_data.original_text, g_strdup(text));
	search_data.flags = 0;
...
G_MODULE_EXPORT void on_insert_date_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	...
	else
	{
		...
		if (str)
			UTILS_REPLACE_PTR(ui_prefs.custom_date_format, str);
		return;
	}
...
G_MODULE_EXPORT void on_menu_open_selected_file1_activate(GtkMenuItem *menuitem, gpointer user_data)
{
	...
	UTILS_REPLACE_PTR(sel, utils_get_locale_from_utf8(sel));
	...
	if (sel != NULL)
	{
		...
		{	/* relative filename, add the path of the current file */
			...
			UTILS_REPLACE_PTR(path, utils_get_locale_from_utf8(path));
			if (!path)
				...
				app->project != NULL && !EMPTY(app->project->base_path))
			{
				...
				UTILS_REPLACE_PTR(path, project_get_base_path());
				UTILS_REPLACE_PTR(path, utils_get_locale_from_utf8(path));
				UTILS_REPLACE_PTR(filename, g_build_path(G_DIR_SEPARATOR_S, path, sel, NULL));
			}
			g_free(path);
#ifdef G_OS_UNIX
			if (! g_file_test(filename, G_FILE_TEST_EXISTS))
				UTILS_REPLACE_PTR(filename, g_build_path(G_DIR_SEPARATOR_S, "/usr/local/include", sel, NULL));

			if (! g_file_test(filename, G_FILE_TEST_EXISTS))
				UTILS_REPLACE_PTR(filename, g_build_path(G_DIR_SEPARATOR_S, "/usr/include", sel, NULL));
#endif
		}
		...
		{
			UTILS_REPLACE_PTR(sel, utils_get_utf8_from_locale(sel));
